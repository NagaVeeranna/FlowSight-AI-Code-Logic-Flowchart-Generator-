...
export const FlowchartViewer: React.FC<FlowchartViewerProps> = ({
  ...
  return (
    <div
      className={`glass-panel flex flex-col h-full border border-slate-200/90 rounded-2xl overflow-hidden shadow-sm transition-all duration-300 ${
        isFullscreen ? 'fixed inset-4 z-50 bg-white/95 backdrop-blur-2xl' : ''
      }`}
    >
      {/* Top Controls Bar */}
      <div className="px-4 py-3 bg-slate-100/90 border-b border-slate-200/90 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Tooltip title="Download Vector SVG" arrow>
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-white border border-slate-300 text-xs font-bold text-slate-800 hover:bg-slate-50 transition-all flex items-center space-x-1.5 shadow-2xs"
              >
            ...
            <Tooltip title="Download High-Res PNG" arrow>
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-indigo-50 border border-indigo-200 text-xs font-bold text-indigo-900 hover:bg-indigo-100 transition-all flex items-center space-x-1.5 shadow-2xs"
              >
      ...
      <div className="flex-1 relative min-h-[350px] bg-[#f8fafc] overflow-hidden flex items-center justify-center p-4">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center max-w-md text-center p-6 bg-rose-50 border border-rose-200 rounded-xl space-y-3">
            ...
            <h4 className="text-sm font-bold text-rose-900">Diagram Rendering Issue</h4>
            <p className="text-xs text-rose-700">{renderError}</p>
            <pre className="w-full text-[11px] font-mono bg-white p-3 rounded text-slate-800 text-left overflow-x-auto max-h-40 border border-rose-200">
              {mermaidCode}
            </pre>
          </div>
        ) : svgContent ? (
          <TransformWrapper initialScale={1} minScale={0.2} maxScale={4} centerOnInit={true}>
            {({ zoomIn, zoomOut, resetTransform }) => (
              <>
                ...
                <div className="absolute top-4 right-4 z-20 flex items-center space-x-1.5 bg-white border border-slate-200/90 p-1.5 rounded-xl shadow-md">
